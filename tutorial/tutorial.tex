...
\begin{frame}[t,fragile]{ALPSセットアップ・スクリプト}
  \begin{itemize}
...
\begin{lstlisting}
$ mkdir $HOME/tutorials
$ cp -rp /usr/share/alps/tutorials/* $HOME/tutorials/
\end{lstlisting}
  \end{enumerate}
\end{frame}
...
\begin{frame}[t,fragile]
  ...
  \item 様々な反強磁性ハイゼンベルグ模型の帯磁率の温度依存性
    ...
    \item tutorial2b.py: 古典ハイゼンベルグ模型(二本足梯子)
    \item tutorial2c.py: $S=1/2$古典ハイゼンベルグ模型(一次元鎖)
    \item tutorial2d.py: $S=1/2$量子ハイゼンベルグ模型(二本足梯子)
    \end{itemize}
  \item 問題
    \begin{itemize}
...
\section{ALPSアプリケーションの実行 / Running ALPS Applications}
\subsection*{{\protect\color{red}●}{\protect\color{blue}●}{\protect\color{green}●}}
