...
\begin{frame}[t,fragile]{ALPS参考資料}
  ...
  \item ALPSチュートリアル資料
    ...
    \item ALPSオフィシャルページ Turorials (en/jp): {\footnotesize \url{http://alps.comp-phys.org/}}
    \end{itemize}
  ...
  \item 藤堂,「``実験技術''としての量子多体シミュレーションソフトウェアALPS」日本物理学会誌掲載予定(jp) \href{http://exa.phys.s.u-tokyo.ac.jp/archive/alps-jps-2015.pdf}{[preprint]}
  \end{itemize}
\end{frame}
...
\begin{frame}[t,fragile]{ワークショップ}
  ...
  \item ALPS users workshop / tutorial
    \begin{itemize}
      ...
      \item CMSIハンズオン: 年4回程度, 神戸や柏にてALPSチュートリアルを開催中
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[t,fragile]{量子格子模型 / Quantum Lattice Models}
  \begin{columns}[T]
...
\begin{frame}[t,fragile]{なぜQLMを考えるのか? / Why quantum lattice models?}
  \begin{columns}[T]
    \begin{column}{.2\textwidth}
...
%\begin{frame}[t,fragile]{実験室で実現されているQLM}
%\end{frame}

%\begin{frame}[t,fragile]{Finite-size scaling analysis}
%\end{frame}
...
\begin{frame}[t,fragile]{ALPSとは? / What's ALPS?}
...
\begin{frame}[t,fragile]{ターゲット・オーディエンス / Target Audience}
  \begin{itemize}
    \setlength{\itemsep}{1em}
  \item 実験家
    \begin{itemize}
